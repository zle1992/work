...
def svm_single(train_data,test_data,train_tags,test_tags): 
#SVM Classifier  
    print ('******************************SVM*****************************' )
    svclf = SVC(kernel = 'rbf',C=1)
    svclf.fit(train_data,train_tags)  
    test_tags_pre = svclf.predict(test_data) 
    # #print(id_list[])
    print('Prognose-Bedarf')
    print(test_tags_pre)

    print ('accuracy_score:{0:.3f}'.format(accuracy_score(test_tags, test_tags_pre)))

def nbrs_single(train_data,test_data,train_tags,test_tags): 
#SVM Classifier  
    print ('******************************SVM*****************************' )
    svclf = SVC(kernel = 'rbf',C=0.5)
    svclf.fit(train_data,train_tags)  
    test_tags_pre = svclf.predict(test_data) 
    #print(id_list[])
    print('预测需求')
